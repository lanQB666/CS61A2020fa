...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    des = make_city('',lat,lon)
    d1,d2 = distance(des,city_a),distance(des,city_b)
    if d1<d2:
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        return True
    judge = [berry_finder(tt) for tt in branches(t)]
    return sum(judge)>0
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        return tree(label(t),[sprout_leaves(tt,leaves) for tt in branches(t)])
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[e,fn(e)] for e in seq if fn(e)>=lower and fn(e)<=upper]

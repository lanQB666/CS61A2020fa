...
def mobile(left, right):
    ...
    return ['mobile', left, right] #detail for mobile
...
def arm(length, mobile_or_planet):
    ...
    return ['arm', length, mobile_or_planet] #detail for arm
...
def total_weight(m): #m is either planet or mobile
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m),"m must be mobile"
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m),'m must be mobile or planet'
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not (lower_bound(y)<=0 and upper_bound(y)>=0),'interval cannot div by 0'
